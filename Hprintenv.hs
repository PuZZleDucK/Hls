...
formatEnvironment opts = do e <- getEnvironment
                            let r = if (targets opts) == [] then e
                                                            else filter (\x -> (elem (fst x) (targets opts))) e
                            return ( (map (\(k,v) -> (stripQuotes(show k))++"="++(stripQuotes(show v))++sep) r))
  where sep = if suppressNewline opts == True then "" else "\n"
...
helpText :: [String]
helpText = [ "Usage: /home/bminerds/x/coreutils/src/printenv [OPTION]... [VARIABLE]..."
...
versionText :: [String]
versionText = [ "Hprintenv (Haskell implementation of GNU printenv) 1.0"
